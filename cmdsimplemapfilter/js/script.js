...
if(Modernizr.webgl) {
	...
	function ready (error, config, geog){
		...
		// Generate list of codes so we can ask the API for the data we want...
		geogcodes = [];
		...
		let query =
			'{"dataset":{"id": "ashe-table-7-earnings","edition": "time-series","version": 1},"dimensions": [{"name": "geography","options":'+ JSON.stringify(geogcodes) + '},{"name": "time","options": ["2017"]},{"name": "sex","options": ["all"]},{"name": "statistics","options": ["median"]},{"name": "earnings","options": ["annual-pay-gross"]},{"name": "workingpattern","options": ["all"]}]}'
			console.log(query)
		...
		d3.csv(data.downloads.csv.href, function(data) {
		...
		data.forEach(function(d) { rateById[d["ashe-geography"]] = +eval("d." + dvc.varname); areaById[d["ashe-geography"]] = d.Geography});
